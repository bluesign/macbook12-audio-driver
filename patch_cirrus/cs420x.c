...
/* CS4208 Cirrus-specific GPIO verbs (different from standard HDA) */
#define CS4208_VERB_SET_GPIO_MASK	0x718
#define CS4208_VERB_SET_GPIO_ENABLE	0x716
#define CS4208_VERB_SET_GPIO_DIR	0x717

/* Forward declarations for MacBook 8,1 setup (defined in patch_cirrus_macbook81_setup.h) */
	...
	snd_hda_gen_update_outputs(codec);
		...
		if (spec->gpio_eapd_hp || spec->gpio_eapd_speaker) {
			if (spec->is_macbook81) {
				/* MacBook 8,1: GPIO4/5 are active-low DFET rails.
				 * Keep both rails on and only toggle mute on GPIO0. */
				if (spec->gen.automute_speaker &&
				    spec->gen.hp_jack_present)
					spec->gpio_data = 0x00; /* DFETs on, muted */
				else
					spec->gpio_data = 0x01; /* DFETs on, unmuted */
			} else if (spec->gen.automute_speaker) {
...
static int cs_init(struct hda_codec *codec)
{
	...
	if (spec->vendor_nid == CS420X_VENDOR_NID) {
		...
		if (spec->is_macbook81) {
			/* MB81: use our own init, not upstream defaults */
			setup_macbook81(codec);
		} else {
	...
	if (spec->gpio_mask) {
		if (spec->vendor_nid == CS4208_VENDOR_NID) {
			snd_hda_codec_write(codec, 0x01, 0,
					    CS4208_VERB_SET_GPIO_MASK,
					    spec->gpio_mask);
			snd_hda_codec_write(codec, 0x01, 0,
					    CS4208_VERB_SET_GPIO_DIR,
					    spec->gpio_dir);
			snd_hda_codec_write(codec, 0x01, 0,
					    CS4208_VERB_SET_GPIO_ENABLE,
					    spec->gpio_mask);
		} else {
	...
	/* MacBook 8,1: re-apply TDM setup after generic init wipes it */
	if (spec->is_macbook81) {
		setup_macbook81(codec);
...
static int cs4208_probe(struct hda_codec *codec)
{
	...
	/* Detect MacBook model by DMI product name */
	if (dmi_match(DMI_PRODUCT_NAME, "MacBook8,1")) {
		codec_info(codec, "MacBook 8,1 detected via DMI\n");
		spec->is_macbook81 = 1;

		/* Keep HP pin 0x10 enabled for headphone output */

		/* Use standard auto-parser (NOT custom PCM ops) */
		...
		/* Pin config overrides BEFORE auto-parser (Windows CONF_08xx + CONF_0807) */
		snd_hda_codec_set_pincfg(codec, 0x10, 0x042b20f0); /* HP */
		snd_hda_codec_set_pincfg(codec, 0x11, 0x50170010); /* LO1: n/c */
		snd_hda_codec_set_pincfg(codec, 0x12, 0x50170011); /* LO2: n/c */
		snd_hda_codec_set_pincfg(codec, 0x13, 0x50170012); /* LO3: n/c */
		snd_hda_codec_set_pincfg(codec, 0x14, 0x50170014); /* LO4: n/c */
		snd_hda_codec_set_pincfg(codec, 0x18, 0x04ab2050); /* HS */
		snd_hda_codec_set_pincfg(codec, 0x19, 0x90a00070); /* DM1 */
		snd_hda_codec_set_pincfg(codec, 0x1d, 0x50400010); /* TX1/SPK: n/c for auto-parser, we drive manually */
		snd_hda_codec_set_pincfg(codec, 0x1e, 0x50560010); /* TX2/0x0b: n/c, kill Digital PCM */
		...
		/* Set GPIO fields so cs_init() preserves DFET state */
		spec->gpio_mask = 0x39; /* GPIO0,3,4,5 enabled */
		spec->gpio_dir  = 0x31; /* GPIO0,4,5 output */
		spec->gpio_data = 0x01; /* DFETs on (GPIO4/5 LOW), unmuted (GPIO0 HIGH) */

		/* NOW apply our TDM setup AFTER auto-parser is done */
		setup_macbook81(codec);
		play_macbook81(codec);

		/* Set playback hook to re-apply TDM on each play */
...
static int cs_mb81_build_pcms(struct hda_codec *codec)
{
	struct cs_spec *spec = codec->spec;
	struct hda_pcm *tdm_info;
	int err;

	/* Let the auto-parser build the standard PCMs first */
	err = snd_hda_gen_build_pcms(codec);
	if (err < 0)
		return err;

	/* Add a second PCM for TDM speaker output (MB81 only) */
	if (spec->is_macbook81) {
