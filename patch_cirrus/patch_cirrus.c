...
static int cs_init(struct hda_codec *codec)
{
	...
	if (spec->gpio_mask) {
		if (spec->vendor_nid == CS4208_VENDOR_NID) {
			/* CS4208 uses Cirrus-specific GPIO verbs */
			snd_hda_codec_write(codec, 0x01, 0, 0x716,
					    spec->gpio_mask);
			snd_hda_codec_write(codec, 0x01, 0, 0x717,
					    spec->gpio_dir);
		} else {
...
#include "patch_cirrus_macbook81_setup.h"
...
static int patch_cs4208(struct hda_codec *codec)
{
	...
	if (dmi_match(DMI_PRODUCT_NAME, "MacBook8,1")) {
		...
		spec->is_macbook81 = 1;
		err = setup_macbook81(codec);
		err = play_macbook81(codec);
	} else {
		/* MacBook 9,1 / 10,1 (A1534) */
		spec->is_macbook81 = 0;
	...
	/* Windows INF: n0AWidgetCaps = 0x00042631 (CCE=1, 4ch) */
	if (spec->is_macbook81) {
		snd_hda_override_wcaps(codec, 0x0a, 0x00042631);
		/* n0ASuppBitsRates = 0x000E0040 (remove R6/32kHz) */
		snd_hdac_override_parm(&codec->core, 0x0a,
				       AC_PAR_PCM, 0x000E0040);
	}
